...
/**
 * E2M MCP Ability Usage Analytics.
 *
 ...
 */
final class E2M_Engine_Analytics
{
    private static ?self $instance = null;
    private const CACHE_GROUP = 'e2m_engine_analytics';
    ...
    private function __construct()
    {
        ...
        add_action('e2m_engine_analytics_cleanup', [$this, 'cleanup_old_logs']);
        add_action('e2m_engine_daily_digest', [$this, 'send_daily_digest']);

        // Schedule cron events only in admin to avoid frontend overhead.
        if (is_admin()) {
            if (!wp_next_scheduled('e2m_engine_analytics_cleanup')) {
                wp_schedule_event(time(), 'daily', 'e2m_engine_analytics_cleanup');
            }
            if (!wp_next_scheduled('e2m_engine_daily_digest')) {
                wp_schedule_event(time(), 'daily', 'e2m_engine_daily_digest');
            }
        }
        ...
        add_action('wp_ajax_e2m_engine_purge_logs', [$this, 'ajax_purge_logs']);
        add_action('wp_ajax_e2m_engine_export_csv', [$this, 'ajax_export_csv']);
        add_action('wp_ajax_e2m_engine_toggle_tracking', [$this, 'ajax_toggle_tracking']);
        add_action('wp_ajax_e2m_engine_get_log_detail', [$this, 'ajax_get_log_detail']);
    }
    ...
    public static function maybe_create_table(): void
    {
        ...
        $installed_ver = (int) get_option('e2m_engine_db_version', 0);
        if ($installed_ver >= self::DB_VERSION) {
            self::$table_verified = true;
            return;
        }

        $table = $wpdb->prefix . 'e2m_engine_logs';
        ...
        update_option('e2m_engine_db_version', self::DB_VERSION, false);
        self::$table_verified = true;
    ...
    private function maybe_send_notification(string $session_id, string $ability_name, ?string $ip, array $settings): void
    {
        ...
        $transient_key = 'e2m_engine_notified_' . md5($session_id);
        if (get_transient($transient_key)) {
            return;
        }

        // Rate limit: max 1 email per 5 minutes to prevent email flooding.
        if (get_transient('e2m_engine_notify_rate_limit')) {
            return;
        }
        ...
        $username = __('Redacted', 'e2mconnect');
        if ($store_user) {
            $user = wp_get_current_user();
            $username = $user->exists() ? $user->user_login : __('Unknown', 'e2mconnect');
        }

        $ip_display = __('Redacted', 'e2mconnect');
        if ($store_ip) {
            $ip_display = $ip ?: __('Unknown', 'e2mconnect');
        }
        ...
        set_transient('e2m_engine_notify_rate_limit', 1, 5 * MINUTE_IN_SECONDS);
    }
    ...
    private function maybe_send_webhook(string $ability_name, string $status, ?string $session_id, array $settings): void
    {
        ...
        if ($secret !== '') {
            $args['headers']['X-E2M-Signature'] = hash_hmac('sha256', $body, $secret);
        }
    ...
    public static function send_test_webhook(string $url, string $secret = ''): array
    {
        $payload = [
            ...
            'site_url'     => home_url(),
            'site_name'    => get_bloginfo('name'),
            'message'      => 'This is a test webhook from E2M MCP.',
        ];
        ...
        return [
            ...
            'message'     => $code >= 200 && $code < 300
                ...
                ? sprintf(__('Success - HTTP %d', 'e2mconnect'), $code)
                /* translators: %d: HTTP status code */
                : sprintf(__('Failed - HTTP %d', 'e2mconnect'), $code),
        ];
    }
    ...
    public function send_daily_digest(): void
    {
        ...
        $body = sprintf(
            ...
            $top_ability ?: '-',
            admin_url('admin.php?page=e2mconnect&tab=activity')
        );
    ...
    {
        return match ($level) {
            'read'   => __('Read', 'e2mconnect'),
            'create' => __('Create', 'e2mconnect'),
            'modify' => __('Modify', 'e2mconnect'),
            'delete' => __('Destructive', 'e2mconnect'),
            default  => __('Unknown', 'e2mconnect'),
        };
    }
    ...
    public static function humanize_ability(string $ability_name): string
    {
        ...
        $map = [
            'e2m-execute-php'          => __('Executed PHP code', 'e2mconnect'),
            'e2m-read-file'            => __('Read a file', 'e2mconnect'),
            'e2m-write-file'           => __('Wrote a file', 'e2mconnect'),
            'e2m-edit-file'            => __('Edited a file', 'e2mconnect'),
            'e2m-delete-file'          => __('Deleted a file', 'e2mconnect'),
            'e2m-list-directory'       => __('Listed directory contents', 'e2mconnect'),
            'e2m-create-page'          => __('Created a new page', 'e2mconnect'),
            'e2m-update-page'          => __('Updated a page', 'e2mconnect'),
            'e2m-list-pages'           => __('Listed pages', 'e2mconnect'),
            'e2m-get-page-structure'   => __('Read page structure', 'e2mconnect'),
            'e2m-build-page'           => __('Built a page with Elementor', 'e2mconnect'),
            'e2m-search-images'        => __('Searched for images', 'e2mconnect'),
            'e2m-sideload-image'       => __('Uploaded an image', 'e2mconnect'),
            'e2m-update-mcp-settings'  => __('Updated MCP settings', 'e2mconnect'),
            'e2m-get-mcp-settings'     => __('Read MCP settings', 'e2mconnect'),
            'e2m-bridge-discover-tools'   => __('Discovered available tools', 'e2mconnect'),
            'e2m-bridge-inspect-tool'     => __('Inspected tool metadata', 'e2mconnect'),
            'e2m-bridge-dispatch-tool'    => __('Dispatched a tool', 'e2mconnect'),
        ];
        ...
        // Auto-generate: "e2m-elementor-update-container" → "Updated Elementor container"
        $clean = str_replace(['e2m-', 'nexter-', 'wdesignkit-', 'mcp-adapter-', 'e2m-bridge-'], '', $ability_name);
    ...
    {
        check_ajax_referer('e2m_engine_analytics_nonce', 'nonce');
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => __('Unauthorized.', 'e2mconnect')]);
        }

        if (!self::ensure_table()) {
            wp_send_json_error(['message' => __('Log table is not available.', 'e2mconnect')]);
        }
        ...
        if ($result === false) {
            wp_send_json_error(['message' => __('Failed to purge logs. Please check DB permissions.', 'e2mconnect')]);
        }

        self::bump_cache_last_changed();

        wp_send_json_success(['message' => __('All log entries have been purged.', 'e2mconnect')]);
    }
    ...
    public function ajax_export_csv(): void
    {
        ...
        if (!wp_verify_nonce($nonce, 'e2m_engine_analytics_nonce')) {
            wp_die(esc_html__('Security check failed.', 'e2mconnect'), 403);
        }
        if (!current_user_can('manage_options')) {
            wp_die(esc_html__('Unauthorized.', 'e2mconnect'), 403);
        }
        ...
        header('Content-Disposition: attachment; filename=e2m-mcp-analytics-' . wp_date('Y-m-d') . '.csv');
        ...
        if ($output === false) {
            wp_die(esc_html__('Failed to create CSV output.', 'e2mconnect'));
        }
    ...
    public function ajax_toggle_tracking(): void
    {
        ...
        wp_send_json_success([
            'enabled' => $enable,
            'message' => $enable
                ? __('MCP Tracking activated.', 'e2mconnect')
                : __('MCP Tracking paused.', 'e2mconnect'),
        ]);
    }
    ...
    public function ajax_get_log_detail(): void
    {
        ...
        if ($log_id <= 0) {
            wp_send_json_error(['message' => __('Invalid log ID.', 'e2mconnect')]);
        }
        ...
        if (!$row) {
            wp_send_json_error(['message' => __('Log entry not found.', 'e2mconnect')]);
        }
