<?php
/**
 * E2M Connect MCP Sandbox Helper.
 *
...
final class E2M_Engine_Sandbox_Helper
{
    ...
    {
        $sandbox_dir = rtrim(E2M_ENGINE_SANDBOX_DIR, '/');
    ...
    public static function maybe_log_syntax_error(string $file, string $message): void
    {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            ...
            error_log('[E2M Connect Sandbox] Syntax error in ' . basename($file) . ': ' . $message);
        }
    }
    ...
    public static function auto_disable_on_syntax_error(string $file, string $reason = ''): void
    {
        ...
        $existing = get_transient('e2m_engine_sandbox_notices');
        if ( ! is_array($existing) ) {
            $existing = [];
        }
        $existing[] = $notice;
        set_transient('e2m_engine_sandbox_notices', $existing, 300);
    }
    ...
    // ── Per-file runtime log ────────────────────────────────────────────
    //
    // Stored as a single autoload=false WP option keyed by the file's path
    // relative to E2M_ENGINE_SANDBOX_DIR. Capped at RUNTIME_CAP entries with
    // LRU eviction so the option never grows unbounded.

    private const RUNTIME_OPTION = 'e2m_engine_sandbox_runtime';
    private const RUNTIME_CAP    = 50;

    /**
     * Record a sandbox-file load result. Call with $error_msg=null on success,
     * or the error text on fatal. On success, count + last_run + duration
     * update; on error, last_error + last_error_at update.
     */
    public static function record_load(string $file, int $duration_ms, ?string $error_msg): void
    {
        if ($file === '') {
            return;
        }

        $key = self::runtime_key($file);
        $log = get_option(self::RUNTIME_OPTION, []);
        if (!is_array($log)) {
            $log = [];
        }
        ...
        if ($error_msg === null) {
            ...
            $entry['last_run']         = time();
            $entry['last_duration_ms'] = max(0, $duration_ms);
        } else {
            $entry['last_error']    = mb_substr($error_msg, 0, 240);
            $entry['last_error_at'] = time();
        }

        $log[$key] = $entry;

        // LRU eviction — drop oldest last_run when over cap.
        if (count($log) > self::RUNTIME_CAP) {
            uasort($log, static fn($a, $b) => (int) ($a['last_run'] ?? 0) <=> (int) ($b['last_run'] ?? 0));
            $log = array_slice($log, -self::RUNTIME_CAP, null, true);
        }

        update_option(self::RUNTIME_OPTION, $log, false);
    }

    /** Return the runtime entry for a given file path, or [] if none. */
    public static function get_runtime(string $file): array
    {
        $log = get_option(self::RUNTIME_OPTION, []);
        if (!is_array($log)) {
            return [];
        ...
        $key = self::runtime_key($file);
        return is_array($log[$key] ?? null) ? $log[$key] : [];
    }

    /** Build the stable storage key for a sandbox file path. */
    private static function runtime_key(string $file): string
    {
        if (!defined('E2M_ENGINE_SANDBOX_DIR')) {
            return basename($file);
        }
        $sandbox = rtrim(E2M_ENGINE_SANDBOX_DIR, '/');
        if (str_starts_with($file, $sandbox . '/')) {
            return ltrim(substr($file, strlen($sandbox)), '/');
        }
        return basename($file);
    }
}
