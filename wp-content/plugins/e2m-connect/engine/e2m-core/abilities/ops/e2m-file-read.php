...
wp_register_ability( 'e2m/read-file', [
    'label'       => __( '[File] Read', 'e2mconnect' ),
    ...
    'execute_callback'    => 'e2m_engine_read_file',
...
function e2m_engine_read_file( array $input ) {
    ...
    // Sensitive-file read guard removed by design - admin tooling needs to
    // be able to read wp-config.php / .env / .git for deep audits, just as
    // they would over SFTP. Path boundary (ABSPATH) is still enforced above.

    if ( ! is_file( $resolved ) ) {
        return new WP_Error( 'e2m_not_file', sprintf( 'Not a regular file: %s', $resolved ) );
    }
    if ( ! is_readable( $resolved ) ) {
        return new WP_Error( 'e2m_file_unreadable', sprintf( 'Cannot read: %s', $resolved ) );
    }

    $file_size = (int) filesize( $resolved );
    $mime      = e2m_engine_detect_mime( $resolved );
    ...
    $envelope = [
        ...
        'size'          => $file_size,
        'mime_type'     => $mime,
        'line_count'    => e2m_engine_count_lines( $resolved ),
    ...
    return $wants_lines
        ? e2m_engine_fetch_lines( $resolved, $input, $envelope )
        : e2m_engine_fetch_bytes( $resolved, $input, $file_size, $mime, $envelope );
}

// Read file content by byte range.

function e2m_engine_fetch_bytes( string $filepath, array $input, int $total_bytes, string $mime, array $envelope ): array|WP_Error {
    ...
    if ( $stream === false ) {
        return new WP_Error( 'e2m_open_failed', sprintf( 'Cannot open: %s', $filepath ) );
    }
    ...
    if ( $raw === false ) {
        return new WP_Error( 'e2m_read_failed', sprintf( 'Read error: %s', $filepath ) );
    }

    $got       = strlen( $raw );
    $was_cut   = $cap !== -1 && ( $skip + $got ) < $total_bytes;
    $text_safe = e2m_engine_content_is_text( $mime, $raw );
...
function e2m_engine_fetch_lines( string $filepath, array $input, array $envelope ): array|WP_Error {

    $rows = file( $filepath );
    if ( $rows === false ) {
        return new WP_Error( 'e2m_read_failed', sprintf( 'Cannot read: %s', $filepath ) );
    }
 ...
 */
function e2m_engine_detect_mime( string $filepath ): string {
 ...
 */
function e2m_engine_count_lines( string $filepath ): int {
    // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
 ...
 */
function e2m_engine_content_is_text( string $mime, string $raw ): bool {
    return e2m_engine_mime_is_textual( $mime ) && mb_check_encoding( $raw, 'UTF-8' );
}
 ...
 */
function e2m_engine_mime_is_textual( string $mime ): bool {
