<?php
/**
 * E2M Connect MCP - Edit File via String Replacement
 *
...
wp_register_ability('e2m/edit-file', [
    'label'       => __('[File] Edit', 'e2mconnect'),
    ...
    'output_schema' => [
        ...
        'properties' => [
            ...
            'backup_id'    => ['type' => 'string',  'description' => 'Rollback id for the pre-edit backup (empty if backups unavailable).'],
        ],
    ],

    'execute_callback'    => 'e2m_engine_edit_file',
 ...
 */
function e2m_engine_edit_file(array $input)
{
    $resolved = e2m_engine_resolve_path((string) $input['path'], require_real: true);
    if (is_wp_error($resolved)) {
        return $resolved;
    }

    // Non-PHP edits can target anywhere under ABSPATH; PHP files are still
    // restricted to the sandbox below.

    $safe_type = e2m_engine_assert_safe_mutable_file_type($resolved);
    if (is_wp_error($safe_type)) {
        return $safe_type;
    }   

    if (!is_file($resolved)) {
        return new WP_Error('e2m_not_file', sprintf('Not an editable file: %s', $resolved));
    }
    // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_is_writable
    if (!is_readable($resolved) || !is_writable($resolved)) {
        return new WP_Error('e2m_not_writable', sprintf('Insufficient permissions: %s', $resolved));
    }
    ...
    if ($old_string === $new_string) {
        return new WP_Error('e2m_no_change', 'Old and new text are identical - nothing to do.');
    }

    // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Local filesystem read.
    $content = file_get_contents($resolved);
    if ($content === false) {
        return new WP_Error('e2m_read_failed', sprintf('Cannot read: %s', $resolved));
    }
    ...
    if ($count > 1 && !$replace_all && $occurrence === null) {
        return new WP_Error('e2m_ambiguous_match', sprintf(
    ...
    if ($replace_all) {
        ...
        $result          = e2m_engine_replace_nth($content, $old_string, $new_string, $occurrence);
        ...
        $first_edit_pos  = strpos($content, $old_string);
        $new_content     = e2m_engine_str_replace_first($content, $old_string, $new_string);
        $replacements    = 1;
    }

    // PHP sandbox + syntax check.
    if (e2m_engine_is_php_extension($resolved)) {
        $sandbox_check = e2m_engine_check_php_sandbox($resolved);
        if (is_wp_error($sandbox_check)) {
            return $sandbox_check;
        }

        // Enforce sandbox max-file-size cap.
        $cap_kb = (int) ((function_exists('e2m_engine_get_settings') ? e2m_engine_get_settings() : [])['sandbox_max_file_size_kb'] ?? 100);
        if ($cap_kb > 0 && strlen($new_content) > $cap_kb * 1024) {
            return new WP_Error(
                'e2m_file_too_large',
                sprintf(
                    ...
                    (int) ceil(strlen($new_content) / 1024),
                    $cap_kb
                )
            );
        }

        $syntax = e2m_engine_edit_validate_sandbox_php($resolved, $new_content);
        if (is_wp_error($syntax)) {
            return $syntax;
        }
    }

    // Safety: back up the current file before we overwrite it.
    $backup_id = '';
    if (class_exists('E2M_File_Backup')) {
        $backup_id = E2M_File_Backup::backup_file($resolved, 'pre:e2m/edit-file');
    }
    ...
    if ($bytes === false) {
        return new WP_Error('e2m_write_failed', sprintf('Cannot write: %s', $resolved));
    }
    ...
    if ($context_lines > 0 && $line_number > 0) {
        $context = e2m_engine_extract_context($new_content, $line_number, $context_lines);
    }
 ...
 */
function e2m_engine_str_replace_first(string $haystack, string $needle, string $replacement): string
{
 ...
 */
function e2m_engine_replace_nth(string $haystack, string $needle, string $replacement, int $n)
{
    $offset = 0;
    for ($i = 1; $i <= $n; $i++) {
        $pos = strpos($haystack, $needle, $offset);
        if ($pos === false) {
            return new WP_Error('e2m_occurrence_missing', sprintf(
    ...
    return new WP_Error('e2m_occurrence_missing', 'Unexpected error in occurrence search.');
}

/**
 * Extract N lines of context around a given line number.
 */
function e2m_engine_extract_context(string $content, int $line_number, int $context): string
{
    $lines = explode("\n", $content);
 ...
 */
function e2m_engine_edit_validate_sandbox_php(string $resolved, string $new_content)
{
    $sandbox_dir  = e2m_engine_get_sandbox_dir(false);
    ...
    return e2m_engine_lint_php($new_content);
}
