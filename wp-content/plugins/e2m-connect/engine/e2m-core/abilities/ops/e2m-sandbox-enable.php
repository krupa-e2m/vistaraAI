<?php
/**
 * E2M Connect MCP - Restore a Paused Sandbox File
 *
...
wp_register_ability('e2m/enable-file', [
    'label'       => __('[File] Enable', 'e2mconnect'),
    ...
    'execute_callback'    => 'e2m_engine_enable_file',
    ...
    'meta' => [
        ...
        'annotations'  => [
            ...
            'instructions' => implode("\n", [
                'USAGE:',
                '• Targets files inside the E2M Connect sandbox directory only.',
 ...
 */
function e2m_engine_enable_file(array $input)
{
    $raw_path   = (string) $input['path'];
    $is_dry_run = !empty($input['dry_run']);

    // Normalise: always look for the .disabled variant first.
    $disabled_candidate = e2m_engine_is_disabled_file($raw_path)
        ? $raw_path
        : $raw_path . '.disabled';

    $resolved = e2m_engine_resolve_path($disabled_candidate, require_real: true);

    // If the .disabled file doesn't exist, check if the active file is already there.
    if (is_wp_error($resolved) && !e2m_engine_is_disabled_file($raw_path)) {
        $active_path = e2m_engine_resolve_path($raw_path, require_real: true);
    ...
    $sandbox_check = e2m_engine_validate_sandbox_path($resolved);
    ...
    if (!e2m_engine_is_disabled_file($resolved)) {
        return [
            'from_path'  => $resolved,
            'to_path'    => $resolved,
            'restored'   => false,
    ...
    if (!rename($resolved, $active_name)) {
        return new WP_Error('e2m_rename_failed', sprintf('Could not rename: %s', $resolved));
    }
