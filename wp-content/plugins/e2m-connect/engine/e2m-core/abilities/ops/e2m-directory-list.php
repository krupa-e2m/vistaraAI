<?php
/**
 * E2M Connect MCP - List Directory Contents
 *
...
wp_register_ability('e2m/list-directory', [
    'label'       => __('[File] List Directory', 'e2mconnect'),
    ...
    'output_schema' => [
        ...
        'properties' => [
            ...
            'policy_tag'  => ['type' => 'string',  'description' => 'E2M directory-list policy marker.'],
        ],
    ],

    'execute_callback'    => 'e2m_engine_list_directory',
    ...
    'meta' => [
        ...
        'annotations'  => [
            ...
            'instructions' => implode("\n", [
                ...
                '',
                'WORKSPACE:',
                '• Use this to inspect files available to E2M-managed workflows.',
            ]),
            'readonly'    => true,
            'destructive' => false,
            'idempotent'  => true,
 ...
 */
function e2m_engine_list_directory(array $input = [])
{
    $plan = e2m_engine_make_directory_query_plan($input);

    $absolute_path = e2m_engine_resolve_path($plan['path'], require_real: true);
    if (is_wp_error($absolute_path)) {
        return $absolute_path;
    }

    if (!is_dir($absolute_path)) {
        return new WP_Error('e2m_not_directory', sprintf('Not a directory: %s', $absolute_path));
    }
    if (!is_readable($absolute_path)) {
        return new WP_Error('e2m_dir_unreadable', sprintf('No read permission: %s', $absolute_path));
    }

    // No sensitive-file guard here by design - admins need to enumerate
    // .git/, .env-bearing dirs, etc. for deep audit workflows, same as
    // they would via SFTP.
    $all_entries = $plan['recursive']
        ? e2m_engine_collect_recursive_entries($absolute_path, $plan['pattern'], $plan['include_hidden'], $plan['max_depth'])
        : e2m_engine_collect_flat_entries($absolute_path, $plan['pattern'], $plan['include_hidden']);
    ...
    e2m_engine_sort_entries($all_entries, $plan['sort_by']);

    $summary = e2m_engine_build_directory_summary($all_entries);
 ...
 */
function e2m_engine_make_directory_query_plan(array $input): array
{
    return [
 ...
 */
function e2m_engine_build_directory_summary(array $entries): array
{
    $total_files = 0;
    $total_dirs  = 0;
    $total_size  = 0;
 ...
 */
function e2m_engine_sort_entries(array &$entries, string $sort_by): void
{
 ...
 * @return list<array>
 */
function e2m_engine_collect_recursive_entries(string $root, string $glob, bool $show_hidden, int $depth_limit): array
{
    ...
    foreach ($tree_iter as $node) {
        $record = e2m_engine_file_info_to_entry($node, $glob, $show_hidden);
        if ($record !== null) {
            $results[] = $record;
        }
    }
 ...
 */
function e2m_engine_collect_flat_entries(string $dir, string $glob, bool $show_hidden)
{
    $handle = opendir($dir);
    if ($handle === false) {
        return new WP_Error('e2m_opendir_failed', sprintf('Cannot open directory: %s', $dir));
    }
 ...
 * @return array|null
 */
function e2m_engine_file_info_to_entry(SplFileInfo $node, string $glob, bool $show_hidden): ?array
{
