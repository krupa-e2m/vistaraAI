 ...
 */
function e2m_engine_get_theme_root_by_scope(string $theme_scope): string
{
 ...
 */
function e2m_engine_resolve_theme_file_path(string $theme_scope, string $relative_path)
{
    $relative_path = ltrim(str_replace('\\', '/', $relative_path), '/');
    if ($relative_path === '') {
        return new WP_Error('missing_relative_path', __('The relative_path parameter is required.', 'e2mconnect'));
    }

    if (str_contains($relative_path, '../') || str_contains($relative_path, '..\\')) {
        return new WP_Error('invalid_relative_path', __('Path traversal is not allowed.', 'e2mconnect'));
    }
    ...
    if (!str_starts_with($absolute, trailingslashit($root))) {
        return new WP_Error('invalid_theme_path', __('The requested path is outside the allowed theme directory.', 'e2mconnect'));
    }
 ...
 */
function e2m_engine_is_supported_theme_extension(string $absolute_path): bool
{
 ...
 */
function e2m_engine_validate_theme_php_syntax(string $content)
{
    $tmp_file = '';

    if (function_exists('wp_tempnam')) {
        $tmp_file = (string) wp_tempnam('e2m-theme-php');
    }

    if ($tmp_file === '') {
        $tmp_dir = function_exists('get_temp_dir') ? get_temp_dir() : sys_get_temp_dir();
        $fallback = tempnam($tmp_dir, 'e2m-theme-php-');
        $tmp_file = $fallback !== false ? $fallback : '';
    }

    if (!$tmp_file) {
        return new WP_Error('temp_file_failed', __('Unable to create a temporary file for PHP validation.', 'e2mconnect'));
    }
    ...
    if ( class_exists( 'E2M_Engine_Sandbox_Helper' ) ) {
        ...
        if ( $binary !== '' ) {
            $lint = E2M_Engine_Sandbox_Helper::lint_php_file( $tmp_file, $binary );
            wp_delete_file( $tmp_file );
            if ( ! $lint['ok'] ) {
                return new WP_Error(
                    'php_syntax_error',
                    /* translators: %s: lint error details */
                    sprintf( __( 'PHP syntax validation failed: %s', 'e2mconnect' ), $lint['message'] )
                );
            }
            return true;
        }
 ...
 * @return string|WP_Error
 */
function e2m_engine_get_php_cli_binary()
{
    ...
    return new WP_Error(
        'php_cli_missing',
        __('PHP CLI binary was not found for syntax validation. Set a valid PHP binary path or install PHP CLI.', 'e2mconnect')
    );
}
 ...
 */
function e2m_engine_list_theme_files(string $theme_scope, string $subdir = '', int $max_files = 200)
{
    $base_path = e2m_engine_resolve_theme_file_path($theme_scope, $subdir === '' ? '.' : $subdir);
    if (is_wp_error($base_path)) {
        return $base_path;
    }

    if (!is_dir($base_path)) {
        return new WP_Error('theme_directory_not_found', __('The requested theme directory was not found.', 'e2mconnect'));
    }
